...
> Capstone project — Electronic Engineering, Universidad del Norte.
> Authors: Mariana Barrero · Jeimy Montes · Lucas Romero · Juliana Vásquez
...
┌──────────────────────────────────────────────────────────────────┐
│                         GitHub Actions                           │
│  Daily cron (01:00 COT) → triggers Supabase Edge Function        │
│  → epiguard-cron repo                                            │
└─────────────────────────────┬────────────────────────────────────┘
                              │ HTTP
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                  Supabase (Backend)                              │
│  Edge Function: daily-update    Edge Function: on-demand-predict │
│  PostgreSQL: predictions_continuous · departments                │
└──────┬──────────────────────────────────────────────────────────┘
       │ Fetch climate (NASA POWER, NOAA) + run inference
       ▼
┌─────────────────────────────────────────────────────────────────┐
│        Hugging Face Space (ML inference node)                    │
│  32 pre-trained SARIMAX models (one per department, ~110 MB)     │
│  Returns: 4-week case forecasts + climate-variable contributions  │
└──────┬──────────────────────────────────────────────────────────┘
       │ REST API
       ▼
┌─────────────────────────────────────────────────────────────────┐
│              Next.js 14 Frontend (this repo)                     │
│  MapLibre GL choropleth · Recharts dashboards · PDF reports      │
└─────────────────────────────────────────────────────────────────┘
```
...
| Automation | GitHub Actions (see [epiguard-cron](https://github.com/LucasRomero26/epiguard-cron)) |
...
**Prerequisites:** Node.js 18+, a Supabase project with the `predictions_continuous` and `departments` tables and the `on-demand-predict` Edge Function deployed.
...
| Repo | Purpose |
|---|---|
| [epiguard-cron](https://github.com/LucasRomero26/epiguard-cron) | GitHub Actions workflow that triggers the daily Supabase Edge Function at 01:00 COT, fetching fresh climate data and updating all 32 department predictions |
