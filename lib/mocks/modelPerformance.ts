...
// Deterministic PRNG (mulberry32) so charts are reproducible across reloads
// instead of reshuffling on every render. Kept local to avoid adding a dep.
...
function genScatter(base: number, count: number, dept: string, code: number) {
  const rand = mulberry32(code * 1731 + count);
  return Array.from({ length: count }, () => ({
    actual: Math.round(base * (0.7 + rand() * 0.6)),
    predicted: Math.round(base * (0.75 + rand() * 0.5)),
    department: dept,
    deptCode: code,
  }));
}
...
function genSeries(base: number): { week: string; actual: number; predicted: number }[] {
  const rand = mulberry32(Math.round(base));
  return Array.from({ length: 52 }, (_, i) => {
    const week = `W${String(i + 1).padStart(2, '0')}-2022`;
    const actual = Math.max(0, Math.round(base * (0.8 + Math.sin(i / 6) * 0.3 + rand() * 0.2)));
    const predicted = Math.max(0, Math.round(actual * (0.9 + rand() * 0.2)));
